  ...
  def pbPokemonScreen
    ...
    loop do
      ...
      if !pkmn.egg?
        ...
        commands[cmdNickname = commands.length]   = _INTL("Rename") if !pkmn.foreign?($player)
      end
      ...
      if cmdSummary>=0 && command==cmdSummary
        ...
        itemcommands[cmdMoveItem=itemcommands.length] = _INTL("Move") if pkmn.hasItem? && !pkmn.item.is_mail?
      ...
      elsif cmdNickname>=0 && command==cmdNickname   # Nickname
        nickname = pbEnterPokemonName(_INTL("{1}'s nickname?",pkmn.speciesName),0,Pokemon::MAX_NAME_SIZE,"",pkmn) 
        pkmn.name = nickname!="" ? nickname : pkmn.speciesName
      end
    end
    @scene.pbEndScene
    return nil
