...
class PokemonEvolutionScene
  ...
  def pbStartScreen(pokemon,newspecies)
    ...
    @pokemon.form=getEvolutionForm(@pokemon.species,@pokemon.form)
  ...
  def pbEvolution(cancancel=true)
    ...
    if canceled
      ...
      abilindex= @pokemon.getAbilityList.index(@pokemon.ability)
      ...
      @pokemon.item=nil if removeItem
...
def checkEvoConditions(pokemon,method,condition,evo)
  case method
    ...
    when :Location
      ...
      return evo if $game_map.map_id==condition
  ...
  case method
    ...
    when :LevelRain then return evo if ($game_screen.weather_type==1 || $game_screen.weather_type== 2 || $game_screen.weather_type== 7)
...
def getEvolutionForm(species,form)
  case species
  ...
  when :PIKACHU then return Raichu.include?($game_map.map_id) ? 1 : 0
