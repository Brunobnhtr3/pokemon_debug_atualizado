...
ShitList = [:EXEGGUTOR,:KYUREM,:TANGROWTH,:STEELIX,:AVALUGG,:CLAWITZER,:SWALOT]
...
def pbLoadPokemonBitmapSpecies(pokemon, species, back=false)
  ...
  if (pokemon.form !=0 && (ShitList.include?(species))) || toobig
    if !(species == :STEELIX && pokemon.form == 1) #mega steelix is excused.
      ...
      if pbResolveBitmap(sprintf("Graphics/Battlers/%03d%s%s%s",dexnum,shinytag,backtag,formnumber))
        return RPG::Cache.load_bitmap(sprintf("Graphics/Battlers/%03d%s%s%s",dexnum,shinytag,backtag,formnumber))
      end
    end
  end
  ...
  bitmap = makeShadowBitmap(bitmap, height, height) if pokemon.isShadow?
  bitmap = makePrismBitmap(bitmap, height, height) if pokemon.item == :BLKPRISM
...
def makePrismBitmap(bitmap, width, height)
  ...
  for w in 0...width/2
    ...
    for h in 0...height/2
      ...
      for loc in border
        border2 = []
        bitmap.set_pixel(loc[0],loc[1],col)
        bitmap.set_pixel(loc[0]+1,loc[1],col)
        bitmap.set_pixel(loc[0],loc[1]+1,col)
        bitmap.set_pixel(loc[0]+1,loc[1]+1,col)
        ...
        for loc2 in border2
          bitmap.set_pixel(loc2[0],loc2[1],col2)
          bitmap.set_pixel(loc2[0]+1,loc2[1],col2)
          bitmap.set_pixel(loc2[0],loc2[1]+1,col2)
          bitmap.set_pixel(loc2[0]+1,loc2[1]+1,col2)
        end
      end
...
def makeShadowBitmap(bitmap, width, height)
  for w in 0...width/2
    for h in 0...height/2
      ...
      pixcolor = bitmap.get_pixel(w*2,h*2)
      next if pixcolor.alpha != 255
      rgbsum = 0.2126 * pixcolor.red + 0.7152 * pixcolor.green + 0.0722 * pixcolor.blue
      #rgbsum = ([pixcolor.red, pixcolor.green, pixcolor.blue].max - [pixcolor.red, pixcolor.green, pixcolor.blue].min)/2
      rgb = [rgbsum]*3
      ...
      for loc in border
        border2 = []
        bitmap.set_pixel(loc[0],loc[1],Color.new(220, 90, 255, 200))
        bitmap.set_pixel(loc[0]+1,loc[1],Color.new(220, 90, 255, 200))
        bitmap.set_pixel(loc[0],loc[1]+1,Color.new(220, 90, 255, 200))
        bitmap.set_pixel(loc[0]+1,loc[1]+1,Color.new(220, 90, 255, 200))
        ...
        border2.push([loc[0],loc[1]-2]) if bitmap.get_pixel(loc[0],loc[1]-2).alpha < 200 #up
        for loc2 in border2
          bitmap.set_pixel(loc2[0],loc2[1],Color.new(220, 90, 255, 70))
          bitmap.set_pixel(loc2[0]+1,loc2[1],Color.new(220, 90, 255, 70))
          bitmap.set_pixel(loc2[0],loc2[1]+1,Color.new(220, 90, 255, 70))
          bitmap.set_pixel(loc2[0]+1,loc2[1]+1,Color.new(220, 90, 255, 70))
        end
      end
...
def pbPokemonIconBitmap(pokemon,egg=false)   # pbpokemonbitmap, but for icons
  ...
  form = pokemon.form
  egg = egg ? "egg" : ""
  species = $cache.pkmn[pokemon.species].dexnum #.name.downcase
...
def pbGetTrainerVictoryME(trainer) # can be a PokeBattle_Trainer or an array of PokeBattle_Trainer
  ...
  for i in 0...trainerarray.length
    trainertype=trainerarray[i].trainertype
    if $cache.trainertypes[trainertype]
      music=$cache.trainertypes[trainertype].checkFlag?(:winBGM)
    end
  end
  return pbStringToAudioFile(music) if music && music!=""
...
def pbAddPokemonSilent(pokemon,level=nil,seeform=true,form=0)
...
def pbHasEgg?(pkmn)
  ...
  baby=pbGetBabySpecies(pkmn.species,pkmn.form)
  return true if pkmn.species==baby   # Is a basic species
  baby=pbGetNonIncenseLowestSpecies(baby,pkmn.form)
...
def pbChooseMove(pokemon,variableNumber,nameVarNumber)
  ...
  if ret>=0
    $game_variables[nameVarNumber]=getMoveName(pokemon.moves[ret].id)
  else
