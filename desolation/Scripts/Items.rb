...
def getMoveName(move)
  return PokeBattle_ZMoves::ZMOVENAMES[move] if PokeBattle_ZMoves::ZMOVENAMES.keys.include?(move)
...
def getAbilityName(abil)
  return $cache.abil[abil].name
end

def getAbilityDesc(abil)
  return $cache.abil[abil].desc
end
...
module ItemHandlers
  ...
  MultipleAtOnce=[:EXPCANDYL,:EXPCANDYXL,:EXPCANDYM,:EXPCANDYS,:EXPCANDYXS, :RARECANDY]
