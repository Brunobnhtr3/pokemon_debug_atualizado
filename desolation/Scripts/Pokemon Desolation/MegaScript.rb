...
##so it's easier for everyone
  ...
  for i in 0..$Trainer.party.length-1
    speciesname=getMonName($Trainer.party[i].species).upcase 
  ...
  for i in 0..$Trainer.party.length-1
    if !$Trainer.party[i].item.nil?
      testString = remove_accents(getItemName($Trainer.party[i].item)).upcase!.delete(" ").delete("-")
    end
  ...
  if possibleStones.length>0
    stoneNames=[]
    for i in 0..possibleStones.length-1
      stoneNames<< getItemName(PBItems.const_get(possibleStones[i]))
    end
...
def zMoveScript
  ...
  $game_variables[:MegaStone]=-1
  pbChoosePokemon(300,301)
  if $game_variables[:MegaStone]!=-1
    moves = $Trainer.party[$game_variables[:MegaStone]].moves
    crystals=[]
    ##we iterate through the moves of a mon, find the type of the moves, add the corresponding
    ##crystal to the array
    for i in 0..$Trainer.party[$game_variables[:MegaStone]].moves.length-1
      crystals << typeCrystals[$Trainer.party[$game_variables[:MegaStone]].moves[i].type]
    end
    ##yeet the duplicates
    
    crystals = crystals.uniq
    ##find the relevant species-centric crystals and add them
    if monCrystals.key?($Trainer.party[$game_variables[:MegaStone]].species)
        crystals.concat monCrystals[$Trainer.party[$game_variables[:MegaStone]].species]
    end
    crystalsToRemove= []
    ...
    if crystals.length>0
      crystalNames=[]
      for i in 0..crystals.length-1
        crystalNames<< getItemName(PBItems.const_get(crystals[i]))
      end
