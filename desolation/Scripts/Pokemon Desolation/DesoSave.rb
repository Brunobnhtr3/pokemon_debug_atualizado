class PokemonSaveScene
  def pbStartScreen
    ...
    textColor=["01ACE6,00BEFF"]
    loctext=_INTL("<ac><c3=00BEFF>{1}</c3></ac>",mapname)
    loctext+=_INTL("Player<r><c3=00BEFF>{2}</c3><br>",textColor,$Trainer.name)
    loctext+=_ISPRINTF("Time<r><c3=00BEFF>{2:02d}:{3:02d}</c3><br>",textColor,hour,min)
    loctext+=_INTL("Badges<r><c3=00BEFF>{2}</c3><br>",textColor,$Trainer.numbadges)
    if $Trainer.pokedex.canViewDex
      loctext+=_INTL("Pokédex<r><c3=00BEFF>{2}/{3}</c3><br>",textColor,$Trainer.pokedex.getOwnedCount,$Trainer.pokedex.getSeenCount)
    end
    if $Unidata[:saveslot]>1
      loctext+=_INTL("Save File:<r><c3=00BEFF>{2}</c3><br>",textColor,$Unidata[:saveslot])
    else
      loctext+=_INTL("Save File:<r><c3=00BEFF>1</c3><br>",textColor)
    end
