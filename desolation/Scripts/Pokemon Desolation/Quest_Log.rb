def initQuestLog
  if($game_variables[:QuestLog]).nil? || !($game_variables.kind_of?(Array))
    $game_variables[:QuestLog]            =load_data("Data/quests.dat")
  end   
end
  ...
  #-----------------------------------------------------------------------------
    ...
    loop do
      ...
      if Input.repeat?(Input::LEFT) || Input.repeat?(Input::RIGHT)
        $game_variables[:QuestLogMainOrSide]= !$game_variables[:QuestLogMainOrSide]
        if $game_variables[:QuestLogMainOrSide]
          $game_variables[:QuestLogLastMain]=@sprites["commands"].index
          ...
          @sprites["subheader"]=Window_UnformattedTextPokemon.newWithSize(_INTL("Main Quests"),
          2,14,160,64,@viewport)
          self.pbSetCommands(pbQuestSeen,$game_variables[:QuestLogLastSide])
        else
          $game_variables[:QuestLogLastSide]=@sprites["commands"].index
          ...
          @sprites["subheader"]=Window_UnformattedTextPokemon.newWithSize(_INTL("Side Quests"),
          2,14,160,64,@viewport)
          self.pbSetCommands(pbQuestSeen,$game_variables[:QuestLogLastMain])
        end
        @sprites["subheader"].baseColor   = Color.new(248,248,248)
        @sprites["subheader"].shadowColor = Color.new(0,0,0)
        @sprites["subheader"].windowskin  = nil
  ...
  def pbStartScreen
    ...
    loop do
      ...
      if cmd<0
        ...
        if commands[cmd] != "???"
          if $game_variables[:QuestLogMainOrSide]
            $game_variables[:QuestLogLastSide]=commands.index(commands[cmd])
          else
            $game_variables[:QuestLogLastMain]=commands.index(commands[cmd])
          end
          pbPlayDecisionSE
          pbFadeOutIn(99999) {
...
def pbQuestSeen
  ...
  ###undiscovered
  sort_order =["1","0","2"]
  $game_variables[:QuestLog]= $game_variables[:QuestLog].sort_by {|a| [sort_order.index(a[1].to_s), $game_variables[:QuestLog].index(a)]}
  for i in 0..$game_variables[:QuestLog].length-1
    ...
    if $game_variables[:QuestLog][i][3]==$game_variables[:QuestLogMainOrSide].to_s
      if $game_variables[:QuestLog][i][1].to_i==0
        questSeen.push("???")
      elsif $game_variables[:QuestLog][i][1].to_i==1
        #'\\c[10]'+
        questSeen.push($game_variables[:QuestLog][i][0])
      else#"\c[3]"+
        questSeen.push("[DONE]"+$game_variables[:QuestLog][i][0])
      end
    end
  end
...
def updateQuestLog(start=0)
  tempLog =$game_variables[:QuestLog].clone
  $game_variables[:QuestLog]=load_data("Data/quests.dat")
  if start>$game_variables[:QuestLog].length-1
    start =$game_variables[:QuestLog].length-1
  end
  if start>0
    $game_variables[:QuestLog]=$game_variables[:QuestLog].drop(start)
  end
  for i in 0...$game_variables[:QuestLog].length
    quest= tempLog.detect { |q| q[0] ==$game_variables[:QuestLog][i][0] }
    if quest!=nil
      $game_variables[:QuestLog][i][1]=quest[1]
      for j in 0...quest[2].length
        if !$game_variables[:QuestLog][i][2][j].nil? && !quest[2][j].nil? 
          if $game_variables[:QuestLog][i][2][j][0]!=quest[2][j][0] 
            $game_variables[:QuestLog][i][2][j][0]= quest[2][j][0]
          end
        end
      end
    end
...
def pbSetQuestGoal(title,goal,newstate,shouldrevealnextgoal=false,shouldcomplete=false,nopopup=false)
  ...
  if shouldrevealnextgoal
    if (goal.is_a? Integer) && (goal+1)<quest[2].length
      quest[2][goal+1][0]=1
    elsif (!goal.is_a? Integer) && quest[2].index(goal)<quest[2].length-1
