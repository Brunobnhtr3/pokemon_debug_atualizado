...
class AdvancedPokedexScene
  ...
  def getInfo
    ...
    case genderbyte
      ...
      when :FemZero then genderString = "Always female" # always male
    ...
    # Base Stats
    baseStats= $cache.pkmn[@species].BaseStats
    baseStatsTot=0
    ...
    # Effort Points
    effortPoints= $cache.pkmn[@species].EVs
    effortPointsTot=0
    ...
    # Wild hold item 
    wilditems = {}
    if $cache.pkmn[@species].flags.dig(:WildItemCommon)
      wilditems.store(:common, $cache.pkmn[@species].flags[:WildItemCommon])
    end
    if $cache.pkmn[@species].flags.dig(:WildItemUncommon)
      wilditems.store(:uncommon, $cache.pkmn[@species].flags[:WildItemCommon])
    end
    if $cache.pkmn[@species].flags.dig(:WildItemRare)
      wilditems.store(:rare, $cache.pkmn[@species].flags[:WildItemCommon])
    end
    if wilditems.size == 0
  ...
  def getEvolutionMessage(evolution)
   ...
   def getMachineMoves
    ...
    hms = {}
    tutors = []
    movesArray = $cache.pkmn[@species].compatiblemoves
