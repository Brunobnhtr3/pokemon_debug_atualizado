...
def pbGetEnvironment
  ...
  if $PokemonGlobal && $PokemonGlobal.diving
    ...
    case $game_player.terrain_tag
      ...
      when PBTerrain::Sand        then return :Sand
    end
    return :None
  end
end
...
def pbGenerateWildPokemon(species,level,sos=false)
  if Rejuv
    ...
    return pbLoadWildBoss(species,bossdata) if bossdata[species]
  end
...
Events.onMapSceneChange+=proc{|sender,e|
  ...
  if darkmap
    if $PokemonGlobal.flashUsed || ($game_switches[:EasyHMs_Password] && $PokemonBag.pbQuantity(:TM70)>0 && $Trainer.numbadges>=BADGEFORFLASH)
...
def pbAutoplayOnTransition
  ...
  if $PokemonGlobal.surfing && surfbgm && $Settings.bike_and_surf_music==1
    pbBGMPlay(surfbgm)
  else
    $game_map.autoplayAsCue
  end
