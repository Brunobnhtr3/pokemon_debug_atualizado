...
def pbDayCareGenerateEgg
 ...
 #   raise _INTL("Can't store the egg")
  ...
  for i in 0...2
    ...
    if (parent.item == :POWERWEIGHT)
      ivinherit[i]=PBStats::HP 
      powercount+=1
    end  
    if (parent.item == :POWERBRACER)
      ivinherit[i]=PBStats::ATTACK 
      powercount+=1
    end  
    if (parent.item == :POWERBELT)
      ivinherit[i]=PBStats::DEFENSE 
      powercount+=1
    end  
    if (parent.item == :POWERLENS)
      ivinherit[i]=PBStats::SPATK 
      powercount+=1
    end  
    if (parent.item == :POWERBAND)
      ivinherit[i]=PBStats::SPDEF 
      powercount+=1
    end
    if (parent.item == :POWERANKLET)
  ...
  shinyretries+=2 if $PokemonBag.pbQuantity(:SHINYCHARM)>0
  if shinyretries>0
  ...
  if mother.isFemale? && (mother.ballused) != :MASTERBALL && (mother.ballused) != :CHERISHBALL
    egg.ballused=mother.ballused
  elsif ditto0 || ditto1
     (father.ballused) != :MASTERBALL && (father.ballused) != :CHERISHBALL
