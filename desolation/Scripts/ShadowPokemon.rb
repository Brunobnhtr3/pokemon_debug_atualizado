def pbPurify(pokemon,scene)
  if pokemon.heartgauge==0 && pokemon.shadow
   ...
   # pokemon.giveRibbon(PBRibbons::NATIONAL)
    ...
    oldmoves=[]
    for i in 0...4; oldmoves.push(pokemon.moves[i].move); end
    pokemon.pbUpdateShadowMoves()
    for i in 0...4
      if pokemon.moves[i] !=0 && pokemon.moves[i].move != oldmoves[i]
...
class PokeBattle_Pokemon
  ...
  def makeShadow
    ...
    if $cache.pkmn[self.species].shadowmoves && !$cache.pkmn[self.species].shadowmoves.nil?
      for i in 0...[4,$cache.pkmn[self.species].shadowmoves.length].min
        self.shadowmoves[i]=$cache.pkmn[self.species].shadowmoves[i]
      end
