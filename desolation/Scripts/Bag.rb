...
class PokemonBag_Scene
  ...
  def pbDetermineTMmenu(itemwindow)
    if itemwindow.item.is_a?(Symbol) && (itemwindow.pocket==4 || pbIsTM?(itemwindow.item))
  ...
  def pbGetSortOrderByType
    # Item ids
    tmx, tm, memory, mint, fossil, evup, gems, utilityhold, levelup, nectar, berry, zcrystals, crest, legendhold, application, pokehold, megastones, pprestore, consumehold, keys, story, legendary, questitem, important, incense, plate, evoitem, justsell, niche, overworld, revival, status, general, battlehold, sidequest, healing, typeboost, ball, mail, battleitem, pinchberry, resistberry, evostone = Array.new(43){[]}
