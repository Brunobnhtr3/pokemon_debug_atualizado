class PokeBattle_Battler
  ...
  def pbCanStatus?(showMessages) #catchall true/false for situations where one can't be statused
    ...
    if !self.status.nil? || damagestate.substitute || (effects[:Substitute]>0 && $cache.moves[@battle.lastMoveUsed].checkFlag?(:soundmove) || (self.ability == :COMATOSE && @battle.FE!=:ELECTERRAIN) || pbShieldsUp?)
...
#===============================================================================
  def pbCanConfuse?(showMessages=true)
    return false if isFainted?
    if damagestate.substitute || (@effects[:Substitute]>0 && $cache.moves[@battle.lastMoveUsed].checkFlag?(:soundmove))
      @battle.pbDisplay(_INTL("But it failed!")) if showMessages
      return false
    end
